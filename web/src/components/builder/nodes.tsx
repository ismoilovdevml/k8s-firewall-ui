...
export function TargetNode({ data }: NodeProps) {
  ...
  return (
    <div className="w-[230px] rounded-md border-2 border-accent/70 bg-raised px-3 py-2 shadow-lg shadow-black/40">
...
export function PeerNode({ data, selected }: NodeProps) {
  ...
  const dirColor = card.direction === 'ingress' ? 'text-allow' : 'text-accent'
  return (
    <div
      className={`w-[230px] cursor-pointer rounded-md border bg-surface px-3 py-2 shadow-md shadow-black/30 ${
